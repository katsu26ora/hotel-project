...
// サムネイルリスト調整
function _adjustThumbnails($selector, ads_each_rows, ads) {
    function adjustThumbnail($current, ratio) {
        var thumbnail_height = $current.data('height') * ratio;
        var thumbnail_width = $current.data('width') * ratio + 0;
        $current.css({
            display: 'inline-block',
            width: thumbnail_width + 2 + 'px',
            height: thumbnail_height + 10 + 16 + 'px'
        });
        $current.find('.materials_thumbnail_img').css({
            width: thumbnail_width + 2 + 'px',
            height: thumbnail_height + 2
        });
        $current.find('.materials_thumbnail_name').css({
    ...
    $selector.each(function(index) {
        var row_index = 0;
        var ww = $(this).width() - 17;
        var thumbnails = new Array();
        var current_width = 0;
        var current_spaces = 0;

        // ADコード削除
        $(this).find('.ad_block').remove();

        $(this).find('.materials_thumbnail').each(function(index) {
            var $this = $(this);

            if (current_width + current_spaces + $this.data('width') < ww * 1.25) {
                current_width += $this.data('width');
                current_spaces += 0 + 2 + 4;
                thumbnails.push($this);
            } else {
                var ratio = (ww - current_spaces) / current_width;  // 倍率
                for (var index = 0; index < thumbnails.length; index++) {
                    adjustThumbnail(thumbnails[index], ratio);
                }
        ...
        if (thumbnails.length > 0) {
            var ratio = 1.0;  // 倍率
            if (current_width + current_spaces > ww * 1.00) {
                ratio = (ww - current_spaces) / current_width;  // 倍率
            }
            for (var index = 0; index < thumbnails.length; index++) {
                adjustThumbnail(thumbnails[index], ratio);
            }
...
function isSP() { return $(window).innerWidth() < 640; }
function isTB() { return 640 <= $(window).innerWidth() && $(window).innerWidth() < 960; }
function isPC() { return 960 <= $(window).innerWidth(); }

function adjustThumbnails($selector, rows_limit, ads_pos, ads) {
    if (isPC()) {
        var defwidth = 345;
        
        function adjustThumbnail($current, ratio) {
            var thumbnail_height = defwidth * ratio;
            var thumbnail_width = defwidth * ratio + 0;
            $current.css({
                display: 'inline-block',
                width: thumbnail_width + 'px',
                height: thumbnail_height + 10 + 16 + 'px'
            });
            $current.find('.materials_thumbnail_img_wrapper').css({
                width: thumbnail_width + 'px',
                height: thumbnail_height + 'px'
            });
            if ($current.find('.materials_thumbnail_img').hasClass('portrait')) {
                $current.find('.materials_thumbnail_img').css({
                    width: 'auto',
                    height: (thumbnail_height - 38) + 'px'
                });
            } else if ($current.find('.materials_thumbnail_img').hasClass('landscape')) {
                $current.find('.materials_thumbnail_img').css({
                    width: (thumbnail_width - 38) + 'px',
                    height: 'auto'
                });
            } else {
                $current.find('.materials_thumbnail_img').css({
        ...
        function hideThumbnail($current) {
            $current.css({
                display: 'none'
            });
        }
        ...
        $selector.each(function(index) {
            ...
            $(this).find('.materials_thumbnail').each(function(index) {
                ...
                if ($rows.length < rows_limit) {
                    if (current_width + current_spaces + defwidth < ww * 1.35) {
                        current_width += defwidth;
                        current_spaces += 10;
                        thumbnails.push($this);
                    } else {
                        ratio = (ww - current_spaces) / current_width;  // 倍率
            ...
            if (thumbnails.length > 0) {
                if ($rows.length < rows_limit) {
                    ...
                    $rows.push(thumbnails[0]);
                } else {
                    for (var index = 0; index < thumbnails.length; index++) {
                        hideThumbnail(thumbnails[index]);
                    }
                }
            }
            if ($rows.length > 2 && ads_pos > 0) {
                $rows[Math.floor($rows.length / 2) - 1].before(ads);
            }
        });
    }   // isPC()
    ...
    if (isSP() || isTB()) {
        ...
        function adjustThumbnail($current, ratio) {
            ...
            $current.css({
                display: 'inline-block',
                width: (thumbnail_width - 20) + 'px',
                height: (thumbnail_height - 20) + 10 + 16 + 'px'
            });
            $current.find('.materials_thumbnail_img_wrapper').css({
                width: (thumbnail_width - 20) + 'px',
                height: (thumbnail_height - 20) + 'px'
            });
            if ($current.find('.materials_thumbnail_img').hasClass('portrait')) {
                $current.find('.materials_thumbnail_img').css({
                    width: 'auto',
                    height: (thumbnail_height - 40) + 'px'
                });
            } else if ($current.find('.materials_thumbnail_img').hasClass('landscape')) {
                $current.find('.materials_thumbnail_img').css({
                    width: (thumbnail_width - 40) + 'px',
                    height: 'auto'
                });
            } else {
                $current.find('.materials_thumbnail_img').css({
...
if (typeof onLoadHeader == "function") onLoadHeader($);
if (typeof onLoadFooter == "function") onLoadFooter($);
